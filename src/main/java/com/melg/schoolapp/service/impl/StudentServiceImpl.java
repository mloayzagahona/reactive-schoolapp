...
public class StudentServiceImpl implements IStudentService {
  ...
  public Mono<Void> enrollStudentToSemester(Student student) {
    ...
    Mono<Semester> existingSemesterMono =
        semesterRepository.findById(student.getSemester().getSemesterId());
    ...
    return existingSemesterMono
        .zipWith(
            ...
            (semester, exStud) ->
                Student.builder()
                    .studentId(exStud.getStudentId())
                    .firstName(exStud.getFirstName())
                    .lastName(exStud.getLastName())
                    .nationality(exStud.getNationality())
                    .totalCredits(exStud.getTotalCredits())
                    .semester(semester)
                    .build())
        .flatMap(
            it ->
                databaseClient
                    .sql(
                        "UPDATE STUDENTS SET current_semester_id=:semesterId "
                            + "WHERE student_id=:studentId;")
                    .bind("semesterId", it.getSemester().getSemesterId())
                    .bind("studentId", it.getStudentId())
                    .then());
  }

  public static final BiFunction<Row, RowMetadata, RecordDTO> RECORD_MAPPING_FUNCTION =
      (row, rowMetaData) ->
          RecordDTO.builder()
  ...
  public static final String SQL_FINDSUB_STUD_SEM =
      "SELECT r.subject_id as subject_id, s.name as name, r.grade as grade "
  ...
  public Flux<RecordDTO> findSubjectsPerStudentPerSemester(final Long studentId, final Long semesterId) {
    ...
    return databaseClient
        .sql(SQL_FINDSUB_STUD_SEM)
        .bind("studentId", studentId)
        .bind("semesterId",semesterId)
        .map(RECORD_MAPPING_FUNCTION)
        .all();
  }
}
